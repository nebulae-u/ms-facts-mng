...
function SharkAttacksTableHead(props) {
  ...
  const order = useSelector(
    ({ SharkAttackManagement }) => SharkAttackManagement.sharkAttacks.order
  );
  const user = useSelector(({ auth }) => auth.user);
  let T = new MDText(i18n.get(user.locale));
  const classes = useStyles(props);
  const [selectedSharkAttacksMenu, setSelectedSharkAttacksMenu] =
    useState(null);
        ...
        <TableCell padding="checkbox" className="relative pl-4 sm:pl-12">
          ...
          {props.numSelected > 0 && (
            ...
            >
              <IconButton
                aria-owns={
                  selectedSharkAttacksMenu ? "selectedSharkAttacksMenu" : null
                }
                aria-haspopup="true"
              ...
              >
                <MenuList>
                          ...
                          children: (
                            <React.Fragment>
                              <DialogTitle id="alert-dialog-title">
                                {T.translate(
                                  "shark_attacks.remove_dialog_title"
                                )}
                              </DialogTitle>
                              <DialogContent>
                                <DialogContentText id="alert-dialog-description">
                                  {T.translate(
                                    "shark_attacks.remove_dialog_description"
                                  )}
                                </DialogContentText>
                              </DialogContent>
                              <DialogActions>
                                <Button
                                  onClick={() => {
                                    ...
                                    closeSelectedSharkAttacksMenu();
                                  }}
                                  color="primary"
                                >
                                  {T.translate(
                                    "shark_attacks.remove_dialog_no"
                                  )}
                                </Button>
                                <Button
                                  onClick={() => {
                                ...
                                >
                                  {T.translate(
                                    "shark_attacks.remove_dialog_yes"
                                  )}
                                </Button>
                              </DialogActions>
                            </React.Fragment>
                  ...
                  >
                    ...
                    <ListItemText
                      primary={T.translate("shark_attacks.remove")}
                    />
                  </MenuItem>
                </MenuList>
              </Menu>
