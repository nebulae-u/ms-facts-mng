...
function SharkAttacksHeader(props) {
  ...
  const searchTextFilter = useSelector(
    ({ SharkAttackManagement }) =>
      SharkAttackManagement.sharkAttacks.filters.name
  );
  ...
  function handleRequestImportSharkAttacks() {
    dispatch(
      Actions.importSharkAttacks({
        filters,
        order,
        page,
        rowsPerPage,
    ...
    <div className="flex flex-1 w-full items-center justify-between">
      ...
      <div className="flex items-center">
        ...
        <FuseAnimate animation="transition.slideLeftIn" delay={300}>
          <Typography className="hidden sm:flex" variant="h6">
            {T.translate("shark_attacks.shark_attacks")}{" "}
          </Typography>
        </FuseAnimate>
      </div>
      ...
      <div className="flex">
        <FuseAnimate animation="transition.slideRightIn" delay={300}>
          <Button
            component={Link}
            to="/shark-attack-mng/shark-attacks/new"
            className="whitespace-no-wrap mr-5"
            variant="contained"
          >
            <span className="hidden sm:flex">
              {T.translate("shark_attacks.add_new_shark_attack")}
            </span>
            <span className="flex sm:hidden">
              {T.translate("shark_attacks.add_new_shark_attack_short")}
            </span>
          </Button>
        </FuseAnimate>
        <FuseAnimate animation="transition.slideRightIn" delay={300}>
          ...
          >
            <span className="hidden sm:flex">
              {T.translate("shark_attacks.import_shark_attack")}
            </span>
            <span className="flex sm:hidden">
              {T.translate("shark_attacks.import_shark_attack_short")}
            </span>
          </Button>
        </FuseAnimate>
      </div>
