...
const defaultData = {
  ...
  fatal_y_n: "Y",
  time: "15:25",
  species: "Tiburon blanco",
  investigator_or_source: "Salvavidas",
  pdf: "pdf",
  href_formula: "formula",
  href: "www.link",
  case_number: "100",
  case_number0: "1011",
};
...
function SharkAttack(props) {
  ...
  const gqlSharkAttack = FactsMngSharkAttack({
    id: props.match.params.sharkAttackId,
  });
  const [readSharkAttack, readSharkAttackResult] = useLazyQuery(
    gqlSharkAttack.query,
    { fetchPolicy: gqlSharkAttack.fetchPolicy }
  );
  const [createSharkAttack, createSharkAttackResult] = useMutation(
    FactsMngCreateSharkAttack({}).mutation
  );
  const [updateSharkAttack, updateSharkAttackResult] = useMutation(
    FactsMngUpdateSharkAttack({}).mutation
  );
    ...
    function updateSharkAttackState() {
      ...
      if (sharkAttackId !== "new") {
        if (
          ...
          loggedUser.selectedOrganization.id !== ""
        ) {
          readSharkAttack({
            variables: {
  ...
  useEffect(() => {
    if (
      createSharkAttackResult.data &&
      createSharkAttackResult.data.FactsMngCreateSharkAttack
    ) {
      setSharkAttack(createSharkAttackResult.data.FactsMngCreateSharkAttack);
      props.history.push(
        "/shark-attack-mng/shark-attacks/" +
          createSharkAttackResult.data.FactsMngCreateSharkAttack.id +
          "/"
      );
      dispatch(
        AppActions.showMessage({
          message: T.translate("shark_attack.create_success"),
          variant: "success",
        })
      );
    }
  ...
  useEffect(() => {
    if (onSharkAttackModifiedResult.data) {
      ...
      dispatch(
        AppActions.showMessage({
          message: T.translate("shark_attack.update_success"),
          variant: "success",
        })
      );
    }
  ...
  useEffect(() => {
    if (
      (sharkAttack && !form) ||
      (sharkAttack && form && sharkAttack.id !== form.id)
    ) {
      setForm(sharkAttack);
    }
  ...
  function canBeSubmitted() {
    return (
      ...
      _.isEmpty(errors) &&
      !_.isEqual(
        { ...sharkAttack, metadata: undefined },
  ...
  if (gqlError) {
    ...
    if (
      !firstErrorMessage.includes ||
      !firstErrorMessage.includes("Cannot return null")
    ) {
      return (
        <Error500Page
          message={T.translate("shark_attack.internal_server_error")}
          description={gqlError.graphQLErrors.map(
            (e) => `@${e.path[0]} => code ${e.message.code}: ${e.message.name}`
          )}
        />
      );
    }
  ...
  if (
    props.match.params.sharkAttackId !== "new" &&
    !readSharkAttackResult.data
  ) {
    return <Error404Page message={T.translate("shark_attack.not_found")} />;
  }

  function queryFactsMngSharkAttackDetails() {
    readSharkAttackDetails({
      variables: {
        organizationId: loggedUser.selectedOrganization.id,
        country: form.country || "NA",
      },
    });
  }
            ...
            <div className="flex flex-col items-start max-w-full">
              <FuseAnimate animation="transition.slideRightIn" delay={300}>
                <Typography
                  ...
                  component={Link}
                  role="button"
                  to="/shark-attack-mng/shark-attacks"
                  color="inherit"
                >
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <FuseAnimate animation="transition.slideLeftIn" delay={300}>
                    <Typography variant="caption">
                      {T.translate("shark_attack.shark_attack_detail")}
                    </Typography>
                  </FuseAnimate>
                </div>
              </div>
      ...
      contentToolbar={
        ...
        >
          <Tab
            ...
            label={T.translate("shark_attack.basic_info")}
          />

          {form && form.metadata && (
            <Tab
              className="h-64 normal-case"
              label={T.translate("shark_attack.metadata_tab")}
            />
          )}
        </Tabs>
      }
